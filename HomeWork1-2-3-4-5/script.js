async function fetchData(cardLimit) {
    try {
...
function renderProducts(data, cardLimit) {
    const productBox__content = document.querySelector(".product-box__content");

    for (let i = 0; i < cardLimit && i < data.length; i++) {
        const { id, name, link, image, text, price, size, color } = data[i];

        const productEl = `
            <div id=${id} class="product__item">
                <div class="item-img" style="background-image: url(${image})">
                    <div class="item-img__hover">
                        <button  class="item-img__hover-btn"><svg width="27" height="25"
                ...
                <div class="product__item__content">
                    ...
                    <p class="product__item__text">${text}</p>
                    <p class="product__item__price">${price}</p>
                </div>
            </div>`;
        productBox__content.insertAdjacentHTML("beforeend", productEl);
    }
    ...
    let count = 0;
    updateCartCount(countCart, count);
...
async function addToCart(event) {
    if (event.target.classList.contains('item-img__hover-btn')) {
        const productItem = event.target.closest('.product__item');
        console.log('Добавлено в корзину:', productItem.id);
        ...
        try {
            ...
            const data = await response.json();
            const desiredObject = data.find(obj => obj.id === productItem.id);
            console.log(desiredObject);
            RenderCart(desiredObject);
...
function updateCartCount(countCart, count) {
    countCart.setAttribute("data-count", count);
}


function RenderCart(desiredObject) {

    const shopCart = document.querySelector('.cart-item');

    if (desiredObject) {
        const productCart = `
        <div class="cart__card">
                <img src=${desiredObject.image} alt="card_img"
                    class="card__img">
                <div class="cart__card-text">
                    <button class="btn__del"><svg class="cart__card__icon" width="18" height="18"
                    viewBox="0 0 18 18"
                    ...
                    <h3 class="cart__card__title">${desiredObject.name}</h3>
                    <span>Price: <span class="text__decor">$${desiredObject.price}</span></span>
                    <span>Color: ${desiredObject.color}</span>
                    <span>Size: ${desiredObject.size}</span>
                    <span class="quantity">Quantity: <input
                            class="quantity__input" type="number"
                            min="0" max="50" value="1"></span>
                </div>

            </div>
        `
        shopCart.insertAdjacentHTML('beforeend', productCart);
    }

    const btns = document.querySelectorAll(".btn__del");
    btns.forEach((el) => {
        el.addEventListener("click", () => {
            const product = el.closest(".cart__card");
            product.remove();
        });
    });
}
